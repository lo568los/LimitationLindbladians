...
def integral2(i,k,s,tb,gamma,eigenergies,limit_value = 700,b=50):
    freq=eigenergies[k]-eigenergies[i]
    if( np.absolute(freq) >= 1/10**10):
        integral = (-1.0j/(2*np.pi))*integrate.quad(spectral_bath,0,b,args=(s,tb,gamma),limit=limit_value,weight='cauchy',wvar=eigenergies[k]-eigenergies[i])[0]
    else:
        integral = (-1.0j/(2*np.pi))*integrate.quad(spectral_bath_2,0,b,args=(s,tb,gamma),limit=limit_value)[0]
    return integral

def C(i,k,s,tb,beta,mu,gamma,eigenergies):
    val = integral1(i,k,s,tb,beta,mu,gamma,eigenergies) + 0.5*(func1(eigenergies[k]-eigenergies[i],s,tb,beta,mu,gamma))

    return val

def D(i,k,s,tb,beta,mu,gamma,eigenergies):
    val = integral1(i,k,s,tb,beta,mu,gamma,eigenergies) + integral2(i,k,s,tb,gamma,eigenergies) + 0.5*(spectral_bath(eigenergies[k]-eigenergies[i],s,tb,gamma)+func1(eigenergies[k]-eigenergies[i],s,tb,beta,mu,gamma))
    return val
...
def re_ness_g(beta_r,beta_l,s,g,ham_type,e):
    ...
    for i in range(number):
            for k in range(number):
                    ...
                    if np.abs(freq) >= 1/10**10:
                        constant12[i,k]=(integral12[i,k]+integral11[i,k])/(np.abs(freq)**(s-1))+(0.5*(spectral_bath(eigenergies[k]-eigenergies[i],s,tb,gamma1))+func1(eigenergies[k]-eigenergies[i],s,tb,beta2,mu2,gamma1))/(np.abs(freq)**(s-1))   #full coefficient created this is nbar+1
                        constant11[i,k]=integral11[i,k]/(np.abs(freq)**(s-1))+(0.5*func1(eigenergies[k]-eigenergies[i],s,tb,beta2,mu2,gamma1)) /(np.abs(freq)**(s-1))                                   # the full coefficient is created
                        
                        constant22[i,k]=(integral22[i,k]+integral21[i,k])/(np.abs(freq)**(s-1))+(0.5*(spectral_bath(eigenergies[k]-eigenergies[i],s,tb,gamma2)+func1(eigenergies[k]-eigenergies[i],s,tb,beta1,mu1,gamma2)) )/(np.abs(freq)**(s-1))    #full coefficient created this is nbar+1
                        constant21[i,k]=integral21[i,k]/(np.abs(freq)**(s-1))+0.5*func1(eigenergies[k]-eigenergies[i],s,tb,beta1,mu1,gamma2)/(np.abs(freq)**(s-1))    # the full coefficient is created
    ...
    scipy.io.savemat(f'ness_data_new2_NL1={NL1},NL2={NL2},NM={NM},e={e:.2f},beta_r={beta_r:.1f},beta_l={beta_l:.1f},g={g:.4f},s={s:.2f}.mat',data_dict)
