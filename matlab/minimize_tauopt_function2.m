...
function [output] = minimize_tauopt_function2(NL,NM,w0list,glist,beta)
    ...
    for index = 1:length_F
        F{index} = kron(eye(dM)/sqrt(dM),F{index});
    end
    ...
    cvx_begin sdp quiet
        ...
        for i = 1:d
               objfunc = objfunc+ abs(V(:,i)'*create_L2_2(rho_th,H_S,H_LS,gamma_matrix,F,NL,NM)*V(:,i)) ;
        end
        minimize objfunc
    ...
    for i = 1:d
        diag_values(i) = V(:,i)'*create_L2_2(rho_th,H_S,H_LS,gamma_matrix,F,NL,NM)*V(:,i);
    end
